...
//#include "SystemProfile.h" //FERNANDO
#include "Compiler.h"
...
//DIA 12_7_11 MODIFICADO PARA FUNCIONAR CON UART 1B <<<<<<<<<<<<<<<<<<<<<<<<<<
...
{
    #ifdef __PIC32MX__ 
        //OpenUART1(UART_EN, (1 << 12)|UART_TX_ENABLE, (CLOCK_FREQ/(1<<mOSCGetPBDIV())/16)/BAUD_RATE-1); //OpenUART2(UART_EN, (1 << 12)|UART_TX_ENABLE, (CLOCK_FREQ/(1<<mOSCGetPBDIV())/16)/BAUD_RATE-1); //FERNANDO //DIA 12_7_11
	
//		//DIA 12_7_11 AÑADIDO EN SUSTITUCIÓN DE LO ANTERIOR... //se tiene que configurar antes...
//    	UARTConfigure(UART1B, UART_ENABLE_PINS_TX_RX_ONLY);
//    	UARTSetFifoMode(UART1B, UART_INTERRUPT_ON_TX_NOT_FULL | UART_INTERRUPT_ON_RX_NOT_EMPTY);
//    	UARTSetLineControl(UART1B, UART_DATA_SIZE_8_BITS | UART_PARITY_NONE | UART_STOP_BITS_1);
//    	UARTSetDataRate(UART1B, GetPeripheralClock(), DESIRED_BAUDRATE);
//   		UARTEnable(UART1B, UART_ENABLE_FLAGS(UART_PERIPHERAL | UART_RX | UART_TX));
//		// Configure UART1B RX Interrupt //DIA 11_7_11 He leido por un foro que la uart 1B equilae a la uart 4...
//		IEC2SET = (0x00000010); //DIA 11_7_11
//		IPC12SET = (0x00000C00); //DIA 11_7_11 Pone la prioridad (bits 12,11,10 a valor 3... ) sin modificar el resto
    #else    
...
//        U2MODEbits.UARTEN = 1;
//        U2STAbits.UTXEN = 1;
		U1ABRG   = (CLOCK_FREQ/2/16)/BAUD_RATE-1; //FERNANDO
        IFS1bits.U1ARXIF = 0; //FERNANDO 
        U1ASTA  = 0; //FERNANDO
        U1AMODE = 0b0000000010000000; //FERNANDO
        U1AMODEbits.UARTEN = 1; //FERNANDO
        U1ASTAbits.UTXEN = 1; //FERNANDO
    #endif
}
...
{
    while(U1BSTAbits.TRMT == 0);//while(U1ASTAbits.TRMT == 0);//while(U2STAbits.TRMT == 0);//FERNANDO //DIA 12_7_11
    U1BTXREG = c;//U1ATXREG = c;//U2TXREG = c;//FERNANDO //DIA 12_7_11
}
...
BYTE ConsoleGet(void)
{
    ...
    while(IFS2bits.U1BRXIF == 0);//while(IFS0bits.U1ARXIF == 0);//while(IFS1bits.U2RXIF == 0);//FERNANDO  //DIA 12_7_11

	Temp = U1BRXREG;//Temp = U1ARXREG;//Temp = U2RXREG;//FERNANDO  //DIA 12_7_11
    IFS2bits.U1BRXIF = 0;//IFS0bits.U1ARXIF = 0;//IFS1bits.U2RXIF = 0;//FENRANDO  //DIA 12_7_11
	return Temp;
}
