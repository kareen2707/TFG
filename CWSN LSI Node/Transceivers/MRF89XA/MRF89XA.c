/*******************************************************************************
* FileName:     MRF89XA.c                     ¡¡¡MODIFIED FROM ORIGINAL STACK!!!
* Dependencies:    
...
#include "Transceivers/Transceivers.h"
    ...
    #endif
        volatile MRF89XA_RX_PACKET          RxPacket[BANK_SIZE];
    #if defined(__18CXX)
        #pragma udata
    #endif
    ...
    ROM BYTE SVALUE[]    = {CHANNEL1_SVALUE, CHANNEL2_SVALUE, CHANNEL3_SVALUE, CHANNEL4_SVALUE, CHANNEL5_SVALUE, CHANNEL6_SVALUE,
        ...
        volatile    BOOL hasAck = FALSE;
            ...
            MRF89XA_ACK_INFO    AckInfo[ACK_INFO_SIZE];
        #endif
    #endif
    ...
    #endif
    volatile BYTE        MACTxBuffer[MRF89XA_TX_PCKT_SIZE];
    #if defined(__18CXX)
        #pragma udata
    #endif
...
TX_END_HERE: 
     ...
     *
     *****************************************************************************************/ 
    /* Commented as it is a dummy function for this transceiver. It won't be
     * called by the upper layer... */
    //BOOL MiMAC_SetAltAddress(INPUT BYTE *Address, INPUT BYTE *PANID){return FALSE;}
     ...
     *      MiMAC_MRF89XA_SetPower(TX_POWER_0_DB);
     *      </code>
     *
     * Remarks:    
     *      None
     *
     **************************************************************************/ 
    BOOL MiMAC_MRF89XA_SetPower(INPUT BYTE outputPower)
    {
        if( outputPower > TX_POWER_N_8DB )
        {
            return FALSE;
        }
...
void SetRFMode(BYTE mode)
{
     ...
     * Example:
     *      <code>
     *      MiMAC_MRF89XA_SendPacket(transParam, MACPayload, MACPayloadLen);
     *      </code>
     *
     * Remarks:    
     *      None
     *
     *****************************************************************************************/   
    BOOL MiMAC_MRF89XA_SendPacket( INPUT MAC_TRANS_PARAM transParam,
                        ...
                        INPUT BYTE MACPayloadLen)
    {
        ...
        if( MACPayloadLen > MRF89XA_TX_BUF_SIZE )
        {
            return FALSE;
        }
    ...
    BOOL MiMAC_ReceivedPacket(void)
    {
        ...
        for(i = 0; i < BANK_SIZE; i++)
        {
            if( RxPacket[i].flags.bits.Valid )
            {
                ...
                MRF89XA_MACRxPacket.flags.Val = RxPacket[i].Payload[0];
                MRF89XA_MACRxPacket.PayloadLen = RxPacket[i].PayloadLen;
                PayloadIndex = 2;
                
                if( MRF89XA_MACRxPacket.flags.bits.destPrsnt )
                {
                    PayloadIndex += MACInitParams.actionFlags.bits.PAddrLength;
                }
                
                if( MRF89XA_MACRxPacket.flags.bits.sourcePrsnt )
                {
                    MRF89XA_MACRxPacket.SourceAddress = (BYTE *)&(RxPacket[i].Payload[PayloadIndex]);
                    PayloadIndex += MACInitParams.actionFlags.bits.PAddrLength;
                }
                else
                {
                    MRF89XA_MACRxPacket.SourceAddress = NULL;
                }
                
                
                #if defined(ENABLE_SECURITY)
                    if( MRF89XA_MACRxPacket.flags.bits.secEn )
                    {
                        ...
                        if( MRF89XA_MACRxPacket.flags.bits.sourcePrsnt )
                        {
                            ...
                            for(j = 0; j < CONNECTION_SIZE; j++)
                            {
                                if( (ConnectionTable[j].status.bits.isValid) && 
                                    isSameAddress(ConnectionTable[j].Address, MRF89XA_MACRxPacket.SourceAddress) )
                                {
                                    break;
                                }
                            }
                        ...
                        #endif
                        MRF89XA_MACRxPacket.PayloadLen -= (PayloadIndex + SEC_MIC_LEN);
                    }   
                    else
                    {
                        MRF89XA_MACRxPacket.PayloadLen -= PayloadIndex;
                    }
                ...
                MRF89XA_MACRxPacket.Payload = (BYTE *)&(RxPacket[i].Payload[PayloadIndex]);
                #if !defined(TARGET_SMALL)    
                    MRF89XA_MACRxPacket.RSSIValue = RSSILock;
                #endif
    ...
    void MiMAC_DiscardPacket(void)
    {
        ...
        /************************************************************************************
         * Function:
         *      BYTE MiMAC_MRF89XA_ChannelAssessment(BYTE AssessmentMode)
         *
         * Summary:
         ...
         * Example:
         *      <code>
         *      NoiseLevel = MiMAC_MRF89XA_ChannelAssessment(CHANNEL_ASSESSMENT_CARRIER_SENSE);
         *      </code>
         *
         * Remarks:    
         *      None
         ...
         *      MiMAC_MRF89XA_PowerState(POWER_STATE_DEEP_SLEEP);
         *      // Put MCU to sleep
         *      Sleep();
         *      // Wake up the MCU by WDT, external interrupt or any other means
         *
         *      // Wake up the RF transceiver
         *      MiMAC_MRF89XA_PowerState(POWER_STATE_OPERATE);
         *      </code>
         *
         * Remarks:    
         *      None
         *
         *****************************************************************************************/ 
        BOOL MiMAC_MRF89XA_PowerState(INPUT BYTE PowerState)
        {
            switch(PowerState)
            {
    ...
    #if !defined(__18CXX)
    {
//COMENTO ESTO. PESE A INHABILITAR LA OPCIÓN DE USAR 2 INTERRUPCIONES AQUÍ HAY CÓDIGO DE LA INHABILITADA :S//        if(PHY_IRQ0 && PHY_IRQ0_En)
//COMENTO ESTO. PESE A INHABILITAR LA OPCIÓN DE USAR 2 INTERRUPCIONES AQUÍ HAY CÓDIGO DE LA INHABILITADA :S//            PHY_IRQ0 = 0;
    ...
    #endif
    {
        if( PHY_IRQ1 && PHY_IRQ1_En )
        {
            if(RF_Mode == RF_RECEIVER)
            {
                ...
                if( PacketLen >= MRF89XA_RX_PCKT_SIZE || PacketLen == 0 || (BankIndex >= BANK_SIZE && (bAck==FALSE)) )
                {
IGNORE_HERE:       
                    {
