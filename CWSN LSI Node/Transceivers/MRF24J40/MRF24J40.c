/*******************************************************************************
* FileName:     MRF24J40.c                    ¡¡¡MODIFIED FROM ORIGINAL STACK!!!
* Dependencies:    
...
#include "Transceivers/Transceivers.h"
    ...
    #include "WirelessProtocols/NVM.h"
        ...
        #define MRF24J40_SPIPut(v)   SPIPut(1, v)
        #define MRF24J40_SPIGet()    SPIGet(1)
    #elif defined MRF24J40_IN_SPI2
        #define MRF24J40_SPIPut(v)   SPIPut(2, v)
        #define MRF24J40_SPIGet()    SPIGet(2)
    #elif defined MRF24J40_IN_SPI3
        #define MRF24J40_SPIPut(v)   SPIPut(3, v)
        #define MRF24J40_SPIGet()    SPIGet(3)
    #elif defined MRF24J40_IN_SPI4
        #define MRF24J40_SPIPut(v)   SPIPut(4, v)
        #define MRF24J40_SPIGet()    SPIGet(4)
    #endif
    ...
    #define FAILURE_COUNTER ((WORD)0x20 * MIPS)
            ...
            ROM unsigned char mySecurityKey[16] = {SECURITY_KEY_00, SECURITY_KEY_01, SECURITY_KEY_02,
                                                   SECURITY_KEY_03, SECURITY_KEY_04, SECURITY_KEY_05,
                                                   SECURITY_KEY_06, SECURITY_KEY_07, SECURITY_KEY_08,
                                                   SECURITY_KEY_09, SECURITY_KEY_10, SECURITY_KEY_11,
                                                   SECURITY_KEY_12, SECURITY_KEY_13, SECURITY_KEY_14,
                                                   SECURITY_KEY_15};
            // The 16-byte security key used in the security module.
        #if defined(__18CXX)    	                                                                             
            #pragma romdata
        #endif
        ROM unsigned char mySecurityLevel = MRF24J40_SECURITY_LEVEL; // The security level used in the security module.
        // The security level is defined in IEEE 802.15.4 specification. It can be from 1 to 7.
        ROM unsigned char myKeySequenceNumber = KEY_SEQUENCE_NUMBER;    
        // The sequence number of security key. Used to identify the security key
	    ...
	    struct {
	        BYTE PayloadLen;
	        BYTE Payload[MRF24J40_RX_PCKT_SIZE];
	    } RxBuffer[BANK_SIZE]; 
    #if defined(__18CXX)
        #pragma udata
    #endif
    ...
    void PHYSetLongRAMAddr(INPUT WORD address, INPUT BYTE value)
    {
        volatile BYTE tmpMRF24J40_IE = MRF24J40_IE;
        
        MRF24J40_IE = 0;
        PHY_CS = 0;
        MRF24J40_SPIPut((((BYTE)(address>>3))&0x7F)|0x80);
        MRF24J40_SPIPut((((BYTE)(address<<5))&0xE0)|0x10);
        MRF24J40_SPIPut(value);
        PHY_CS = 1;
        MRF24J40_IE = tmpMRF24J40_IE;
    }
    ...
    BYTE PHYGetLongRAMAddr(INPUT WORD address)
    {
        ...
        MRF24J40_IE = 0;
        PHY_CS = 0;
        MRF24J40_SPIPut(((address>>3)&0x7F)|0x80);
        MRF24J40_SPIPut(((address<<5)&0xE0));
        toReturn = MRF24J40_SPIGet();
        PHY_CS = 1;
        MRF24J40_IE = tmpMRF24J40_IE;
    ...
    void InitMRF24J40(void){
        ...
        MiMAC_MRF24J40_SetChannel(MACCurrentChannel, 0);
    ...
    /***************************************************************************
     * Function:    BOOL MiMAC_MRF24J40_ReceivedPacket(void)
     * Summary:     This function check if a new packet has been received by the
     *              RF transceiver
     * Description: This is the primary MiMAC interface for the protocol layer
     *              to check if a packet has been received by the RF transceiver
     *              When a packet has been received, all information will be
     *              stored in the global variable MRF24J40_MACRxPacket in the
     *              format of MAC_RECEIVED_PACKET;
     ...
     * Example:
     *      <code>
     *      if(TRUE == MiMAC_MRF24J40_ReceivedPacket()){
     ...
     * Remarks:     None
     **************************************************************************/ 
    BOOL MiMAC_MRF24J40_ReceivedPacket(void){
        BYTE i;
        ...
        if(BankIndex < BANK_SIZE){
            ...
            #endif
            MRF24J40_MACRxPacket.flags.Val = 0;
            MRF24J40_MACRxPacket.altSourceAddress = FALSE;
            ...
            switch(addrMode){
                case 0xC8: //short dest, long source
                    ...
                    if(RxBuffer[BankIndex].Payload[5] == 0xFF && \
                       RxBuffer[BankIndex].Payload[6] == 0xFF){
                        MRF24J40_MACRxPacket.flags.bits.broadcast = 1;
                    }
                    MRF24J40_MACRxPacket.flags.bits.sourcePrsnt = 1;
                    ...
                    {
                        #ifndef TARGET_SMALL
                            MRF24J40_MACRxPacket.SourcePANID.v[0] = RxBuffer[BankIndex].Payload[3];
                            MRF24J40_MACRxPacket.SourcePANID.v[1] = RxBuffer[BankIndex].Payload[4];
                        #endif
                        MRF24J40_MACRxPacket.SourceAddress = &(RxBuffer[BankIndex].Payload[7]);
                        
                        MRF24J40_MACRxPacket.PayloadLen = RxBuffer[BankIndex].PayloadLen - 19;
                        MRF24J40_MACRxPacket.Payload = &(RxBuffer[BankIndex].Payload[15]);
                    }
                    #ifndef TARGET_SMALL
                        else {
                            MRF24J40_MACRxPacket.SourcePANID.v[0] = RxBuffer[BankIndex].Payload[7];
                            MRF24J40_MACRxPacket.SourcePANID.v[1] = RxBuffer[BankIndex].Payload[8];
                            MRF24J40_MACRxPacket.SourceAddress = &(RxBuffer[BankIndex].Payload[9]);
                            MRF24J40_MACRxPacket.PayloadLen = RxBuffer[BankIndex].PayloadLen - 21;
                            MRF24J40_MACRxPacket.Payload = &(RxBuffer[BankIndex].Payload[17]);
                        }
                    #endif
                    break;
                ...
                case 0xCC: // long dest, long source
                    ...
                    #endif
                    {
                        //rxFrame.flags.bits.intraPAN = 1;
                            ...
                            MRF24J40_MACRxPacket.SourcePANID.v[1] = RxBuffer[BankIndex].Payload[4];
                        #endif
                        MRF24J40_MACRxPacket.SourceAddress = &(RxBuffer[BankIndex].Payload[13]);
                        MRF24J40_MACRxPacket.PayloadLen = RxBuffer[BankIndex].PayloadLen - 25;
                        MRF24J40_MACRxPacket.Payload = &(RxBuffer[BankIndex].Payload[21]);
                    } 
                    #ifndef TARGET_SMALL
                        else {
                            MRF24J40_MACRxPacket.SourcePANID.v[0] = RxBuffer[BankIndex].Payload[13];
                            MRF24J40_MACRxPacket.SourcePANID.v[1] = RxBuffer[BankIndex].Payload[14];
                            MRF24J40_MACRxPacket.SourceAddress = &(RxBuffer[BankIndex].Payload[15]);
                            MRF24J40_MACRxPacket.PayloadLen = RxBuffer[BankIndex].PayloadLen - 27;
                            MRF24J40_MACRxPacket.Payload = &(RxBuffer[BankIndex].Payload[23]);
                        }
                    #endif
                    break;
                ...
                case 0x80:      // short source only. used in beacon        
                    MRF24J40_MACRxPacket.flags.bits.broadcast = 1;
                    MRF24J40_MACRxPacket.flags.bits.sourcePrsnt = 1;
                    MRF24J40_MACRxPacket.altSourceAddress = TRUE;
                    MRF24J40_MACRxPacket.SourcePANID.v[0] = RxBuffer[BankIndex].Payload[3];
                    MRF24J40_MACRxPacket.SourcePANID.v[1] = RxBuffer[BankIndex].Payload[4];
                    MRF24J40_MACRxPacket.SourceAddress = &(RxBuffer[BankIndex].Payload[5]);
                    MRF24J40_MACRxPacket.PayloadLen = RxBuffer[BankIndex].PayloadLen - 11;
                    MRF24J40_MACRxPacket.Payload = &(RxBuffer[BankIndex].Payload[7]);
                    break;
                ...
                case 0x88: // short dest, short source                
                    ...
                    MRF24J40_MACRxPacket.altSourceAddress = TRUE;
                        ...
                        if(bIntraPAN == FALSE){
                            ...
                            MRF24J40_MACRxPacket.SourcePANID.v[1] = RxBuffer[BankIndex].Payload[8];
                            MRF24J40_MACRxPacket.SourceAddress = &(RxBuffer[BankIndex].Payload[9]);
                            MRF24J40_MACRxPacket.PayloadLen = RxBuffer[BankIndex].PayloadLen - 15;
                            MRF24J40_MACRxPacket.Payload = &(RxBuffer[BankIndex].Payload[11]);
                        }
                        else
                    #endif
                    {
                        ...
                        MRF24J40_MACRxPacket.SourcePANID.v[1] = RxBuffer[BankIndex].Payload[4];
                        MRF24J40_MACRxPacket.SourceAddress = &(RxBuffer[BankIndex].Payload[7]);
                        MRF24J40_MACRxPacket.PayloadLen = RxBuffer[BankIndex].PayloadLen - 13;
                        MRF24J40_MACRxPacket.Payload = &(RxBuffer[BankIndex].Payload[9]);
                    }
                    break;
                ...
                case 0x8C:  // long dest, short source
                    ...
                    #endif
                    {
                            ...
                            MRF24J40_MACRxPacket.SourcePANID.v[1] = RxBuffer[BankIndex].Payload[4];
                        #endif
                        MRF24J40_MACRxPacket.SourceAddress = &(RxBuffer[BankIndex].Payload[12]);
                        ...
                        MRF24J40_MACRxPacket.Payload = &(RxBuffer[BankIndex].Payload[15]);
                            ...
                            MRF24J40_MACRxPacket.SourcePANID.v[0] = RxBuffer[BankIndex].Payload[12];
                            MRF24J40_MACRxPacket.SourcePANID.v[1] = RxBuffer[BankIndex].Payload[13];
                            MRF24J40_MACRxPacket.SourceAddress = &(RxBuffer[BankIndex].Payload[14]);
                            MRF24J40_MACRxPacket.PayloadLen = RxBuffer[BankIndex].PayloadLen - 21;
                            MRF24J40_MACRxPacket.Payload = &(RxBuffer[BankIndex].Payload[17]);
                        }
                    #endif
                ...
                if(RxBuffer[BankIndex].Payload[0] & 0x08){
                    ...
                    switch(mySecurityLevel){
                        case 0x02:
                        case 0x05:
                            MRF24J40_MACRxPacket.PayloadLen -= 21;
                            break;
                        case 0x03:
                        case 0x06:
                            MRF24J40_MACRxPacket.PayloadLen -= 13;
                            break;
                        case 0x04:
                        case 0x07:
                            MRF24J40_MACRxPacket.PayloadLen -= 9;
                            break;
                        case 0x01:
                            MRF24J40_MACRxPacket.PayloadLen -= 5;
                            break;
                        default:
                            break;    
                    }
                    // remove the security header from the payload 
                    MRF24J40_MACRxPacket.Payload = &(MRF24J40_MACRxPacket.Payload[5]);
                    MRF24J40_MACRxPacket.flags.bits.secEn = 1;
                }
            #else
                if(RxBuffer[BankIndex].Payload[0] & 0x08){
                    MiMAC_MRF24J40_DiscardPacket();
            ...
            switch(RxBuffer[BankIndex].Payload[0] & 0x07){ // check frame type
                case 0x01:  // data frame
                    MRF24J40_MACRxPacket.flags.bits.packetType = PACKET_TYPE_DATA;
                    break;
                case 0x03:  // command frame
                    MRF24J40_MACRxPacket.flags.bits.packetType = PACKET_TYPE_COMMAND;
                    break;
                case 0x00:  // use reserved packet type to represent beacon packet
                    MRF24J40_MACRxPacket.flags.bits.packetType = PACKET_TYPE_RESERVE;
                    break;
            ...
            #ifndef TARGET_SMALL
                MRF24J40_MACRxPacket.LQIValue = RxBuffer[BankIndex].Payload[RxBuffer[BankIndex].PayloadLen-2];
                MRF24J40_MACRxPacket.RSSIValue = RxBuffer[BankIndex].Payload[RxBuffer[BankIndex].PayloadLen-1];
            #endif
            return TRUE;
        }
        return FALSE;
    ...
    /***************************************************************************
     * Function:    void MiMAC_MRF24J40_DiscardPacket(void)
     ...
     * Remarks:     None
     *****************************************************************************************/ 
    void MiMAC_MRF24J40_DiscardPacket(void){
        //re-enable the ACKS
        if(BankIndex < BANK_SIZE){
    ...
    /************************************************************************************
     * Function:
     *      BOOL MiMAC_MRF24J40_SendPacket(  MAC_TRANS_PARAM transParam,
     ...
     * Example:
     *      <code>
     *      MiMAC_MRF24J40_SendPacket(transParam, MACPayload, MACPayloadLen);
     *      </code>
     *
     * Remarks:    
     *      None
     *
     *****************************************************************************************/    
    BOOL MiMAC_MRF24J40_SendPacket(INPUT MAC_TRANS_PARAM transParam,
        ...
        /************************************************************************************
         * Function:
         *      BYTE MiMAC_MRF24J40_ChannelAssessment(BYTE AssessmentMode)
         *
         * Summary:
         ...
         * Example:
         *      <code>
         *      NoiseLevel = MiMAC_MRF24J40_ChannelAssessment(CHANNEL_ASSESSMENT_CARRIER_SENSE);
         *      </code>
         *
         * Remarks:    
         *      None
         *
         *****************************************************************************************/     
        BYTE MiMAC_MRF24J40_ChannelAssessment(INPUT BYTE AssessmentMode)
        {
            BYTE RSSIcheck;
        ...
        /************************************************************************************
         * Function:
         *      BOOL MiMAC_MRF24J40_PowerState(BYTE PowerState)
         *
         * Summary:
         ...
         *      MiMAC_MRF24J40_PowerState(POWER_STATE_DEEP_SLEEP);
         *      // Put MCU to sleep
         *      Sleep();
         *      // Wake up the MCU by WDT, external interrupt or any other means
         *
         *      // Wake up the RF transceiver
         *      MiMAC_MRF24J40_PowerState(POWER_STATE_OPERATE);
         *      </code>
         *
         * Remarks:    
         *      None
         *
         *****************************************************************************************/    
        BOOL MiMAC_MRF24J40_PowerState(INPUT BYTE PowerState)
        {
            switch(PowerState)
            {
                ...
                case POWER_STATE_OPERATE:
                    {
                            ...
                            while(1)
                            {
                                ...
                                {
                                    InitMRF24J40();
                                    //MiMAC_MRF24J40_SetChannel(MACCurrentChannel);
                                    MiMAC_SetAltAddress(myNetworkAddress.v, MAC_PANID.v);
                                    break;
                                }
    ...
    /************************************************************************************
     * Function:
     *      BOOL MiMAC_MRF24J40_SetChannel(BYTE channel, BYTE offsetFreq)
     *
     * Summary:
     ...
     *      MiMAC_MRF24J40_SetChannel(12, 0);
     *      </code>
     *
     * Remarks:    
     *      None
     *
     *****************************************************************************************/      
    BOOL MiMAC_MRF24J40_SetChannel(INPUT BYTE channel, INPUT BYTE offsetFreq){
    ...
    /************************************************************************************
     * Function:
     *      BOOL MiMAC_MRF24J40_SetPower(BYTE outputPower)
     *
     * Summary:
     ...
     *      MiMAC_MRF24J40_SetPower(TX_POWER_0_DB);
     *      </code>
     *
     * Remarks:    
     *      None
     *
     *****************************************************************************************/ 
    BOOL MiMAC_MRF24J40_SetPower(INPUT BYTE outputPower){
        BYTE reg;

        //Microchip LSI's  simpler algorithm. MRF24J40's RFCTRL3 register fields
        //are:  TXPWRL1 TXPWRL0 TXPWRS2 TXPWRS1 TXPWRS0 r       r       r
        //      bit7    bit6    bit5    bit4    bit3    bit2    bit1    bit0
        //TXPWRL: large scale value between 0 and -30 dBm, in -10 dBm steps.
        //TXPWRS: short scale value varying between 0 and -6,3 dBm, irregularly.

        //Algorithm: Ignore the 3 least significant bits and write the register.
        //Thus, outputPower grows from 11 111 XXX to 00 000 XXX.
        reg = outputPower & 0xF8;
        //Printf("\rPower at MAC Layer = ");
        //PrintChar(reg);
        
        //Old algorithm. Periodical (P=40), skipping some power output options.
        //Quite strange...
...
//        }
    ...
    /************************************************************************************
     * Function:
     *      BOOL MiMAC_MRF24J40_Init(MACINIT_PARAM initValue)
     *
     * Summary:
     ...
     * Example:
     *      <code>
     *      MiMAC_MRF24J40_Init(initParameter);
     *      </code>
     *
     * Remarks:    
     *      None
     *
     *****************************************************************************************/ 
    BOOL MiMAC_MRF24J40_Init(INPUT MACINIT_PARAM initValue)
    {   
        BYTE i;
     ...
     ********************************************************************/
        ...
        #endif
    #else
        void _ISRFAST _INT3Interrupt(void)
    #endif
    {
        WDTCONCLR = 0x8000;         //Juan: Disable WDT, just in case...
        if(MRF24J40_IE && MRF24J40_IF)
        {  
            BYTE i;
            BYTE j;
            ...
            //flag quickly and then handle the interrupt that we have already received
            {
                ...
                {  
                    ...
                    if( RxBank < BANK_SIZE )
                    {
                        ...
                        if(RxBuffer[RxBank].PayloadLen<MRF24J40_RX_PCKT_SIZE)
                        {   
...
START_OF_SEC_INT:                
        ...
        } //end of if(MRF24J40_IE && MRF24J40_IF)
