/*******************************************************************************
* FileName:     MCHP_MAC.h                      MODIFIED FROM ORIGINAL STACK!!
...
*******************************************************************************/
    ...
    typedef struct {
        union {
            ...
            struct {
                ...
                BYTE secEn       : 1; // 1: secure the MAC payload,
                                      // 0: send plain text
                BYTE repeat      : 1; // 1: allow repeaters to forward the message,
                                      // 0: send message directly
                BYTE ackReq      : 1; // 1: acknowledgement required,
                                      // 0: no acknowldgement
                BYTE destPrsnt   : 1; // 1: destination address in the packet,
                                      // 0: destination address not in the packet
                BYTE sourcePrsnt : 1; // 1: source address in the packet,
                                      // 0: source address not in the packet
            } bits;
        } flags;
        BYTE        *DestAddress;     // destination address
        #if defined(IEEE_802_15_4)
            BOOL     altDestAddr;     // use the alternative network address
                                      // as destination in the packet
            BOOL     altSrcAddr;      // use the alternative network address as
    ...
    typedef struct {
        ...
        #if defined(IEEE_802_15_4)
            BOOL     altSourceAddress;  // Source address is the alternative
                                        // network address
    ...
    typedef struct {
        union {
            BYTE Val;
            struct {
                BYTE RepeaterMode   :1; // 1: Allow to act as repeater,
                                        // 0: disable repeater function
                BYTE CCAEnable      :1; // 1: Enable Clear Channel Assessment,
                                        // 0: Disable CCA
                BYTE NetworkFreezer :1;
                BYTE PAddrLength    :4; // Length of the permanent address,
                                        // range from 1 to 8.
            } bits;   
        } actionFlags; 
    ...
    #if defined MRF24J40
        extern MAC_RECEIVED_PACKET MRF24J40_MACRxPacket;
    #endif
    #if defined MRF49XA_1
        extern MAC_RECEIVED_PACKET MRF49XA_1_MACRxPacket;
    #endif
    #if defined MRF49XA_2
        extern MAC_RECEIVED_PACKET MRF49XA_2_MACRxPacket;
    #endif
    #if defined MRF89XA
        extern MAC_RECEIVED_PACKET MRF89XA_MACRxPacket;
    #endif
...
////////////////////////////////////////////////////////////////////////////////
    ...
    /***************************************************************************
     * Function:    BOOL MiMAC_SetChannel(BYTE channel, BYTE offsetFreq)
     ...
     *                                center frequency. May not apply to all RF
     *                                transceivers
     ...
     **************************************************************************/
        ...
        BOOL MiMAC_MRF49XA_SetChannel(BYTE channel, BYTE offsetFreq, BYTE MRF49XAnum);
    #endif
    #if defined MRF89XA
        BOOL MiMAC_MRF89XA_SetChannel(BYTE channel, BYTE offsetFreq);
    #endif
        
    /***************************************************************************
     * Function:    BOOL MiMAC_SetPower(BYTE outputPower)
    ...
    #endif
    #if defined MRF49XA_1 || defined MRF49XA_2
        BOOL MiMAC_MRF49XA_SetPower(BYTE outputPower, BYTE MRF49XAnum);
    #endif
    #if defined MRF89XA
        BOOL MiMAC_MRF89XA_SetPower(BYTE outputPower);
    #endif
     ...
     * Remarks:     None
     **************************************************************************/ 
    // THIS FUNCTION IS ONLY SUPPORTED IN 2,4 GHz BAND SO IT'S ONLY IMPLEMENTED
    // IN MRF24J40 TRANSCEIVER. THERE IS ONLY ONE FUNCTION FOR THIS PURPOSE
    // INSTEAD OF THREE LIKE IN MOST CASES.
    BOOL MiMAC_SetAltAddress(BYTE *Address, BYTE *PANID);

    /***************************************************************************
     * Function:    BOOL MiMAC_Init(MACINIT_PARAM initValue)
    ...
    #endif
    #if defined MRF49XA_1 || defined MRF49XA_2
        BOOL MiMAC_MRF49XA_Init(MACINIT_PARAM initValue, BYTE MRF49XAnum);
    #endif
    #if defined MRF89XA
        BOOL MiMAC_MRF89XA_Init(MACINIT_PARAM initValue);
    #endif

    /***************************************************************************
     * Function:    BYTE MiMAC_ChannelAssessment(BYTE AssessmentMode)
     * Summary:     This function perform the noise detection on current
     *              operating channel
     * Description: This is the primary MiMAC interface for the protocol layer
     *              to perform the noise detection scan. Not all assessment
     *              modes are supported for all RF transceivers.
     * PreCondition: MiMAC initialization has been done.
     * Parameters:  BYTE AssessmentMode - The mode to perform noise assessment.
     *                                    The possible assessment modes are
    ...
    /***************************************************************************
     * Function:    void MiMAC_DiscardPacket(void)
    ...
    #endif
    #if defined MRF89XA
        void MiMAC_MRF89XA_DiscardPacket(void);;
    #endif

    /***************************************************************************
     * Function:    BOOL MiMAC_ReceivedPacket(void)
     ...
     *              stored in the global variable MACRxPacket in the format of
     *              MAC_RECEIVED_PACKET;
     * PreCondition: MiMAC initialization has been done. 
     * Parameters:  None
    ...
    #endif
    #if defined MRF89XA
        BOOL MiMAC_MRF89XA_ReceivedPacket(void);
    #endif
    
    /***************************************************************************
     * Function:    BOOL MiMAC_SendPacket(MAC_TRANS_PARAM transParam,
    ...
    #endif
    #if defined MRF49XA_1 || defined MRF49XA_2
        BOOL MiMAC_MRF49XA_SendPacket(MAC_TRANS_PARAM transParam,
                        BYTE *MACPayload, BYTE MACPayloadLen, BYTE MRF49XAnum);
    #endif
    #if defined MRF89XA
        BOOL MiMAC_MRF89XA_SendPacket(MAC_TRANS_PARAM transParam,
                                      BYTE *MACPayload, BYTE MACPayloadLen);
    #endif
    
    /***************************************************************************
     * Function:    BOOL MiMAC_PowerState(BYTE PowerState)
     * Summary:     This function puts the RF transceiver into sleep or wake it
     *              up.
     * Description: This is the primary MiMAC interface for the protocol layer
     *              to set different power state for the RF transceiver. There
     *              are minimal power states defined as deep sleep and operating
     *              mode. Additional power states can be defined for individual
     *              RF transceiver depends on hardware design.
     * PreCondition: MiMAC initialization has been done.
     * Parameters:  BYTE PowerState - The power state of the RF transceiver to
     *                                be set to. The minimum definitions for all
     *                                RF transceivers are
     *                  * POWER_STATE_DEEP_SLEEP RF transceiver deep sleep mode.
     *                  * POWER_STATE_OPERATE RF transceiver operating mode.
     * Returns:     A boolean to indicate if chaning power state of RF
    ...
    #endif
    #if defined MRF49XA_1 || defined MRF49XA_2
        BOOL MiMAC_MRF49XA_PowerState(BYTE PowerState, BYTE MRF49XAnum);
    #endif
    #if defined MRF89XA
        BOOL MiMAC_MRF89XA_PowerState(BYTE PowerState);
    #endif
    ...
    void UserInterruptHandler(void);        //PIC18
