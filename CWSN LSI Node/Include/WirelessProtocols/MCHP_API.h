/*******************************************************************************
* FileName:     MCHP_API.h              SLIGHTLY MODIFIED FROM ORIGINAL STACK!!
* Dependencies:
...
*******************************************************************************/
    ...
    //MICROCHIP - LSI Stack: 3 bands for including up to 3 MiWi Transceivers
    ...
    #if defined PROTOCOL_P2P
    //Juan: Despite the only bits used in P2P.c are RXOnWhenIdle and Valid, I
    //do not modify the original definition of this structure...
        BYTE Val;
        struct _CONNECTION_STATUS_bits {
            BYTE RXOnWhenIdle      :1;  //1 = transceiver always on,
                                        //0 = transceiver sleeps when idle
            BYTE directConnection  :1;  //1 = can talk to this device directly,
                                        //0 = must route to this device
            BYTE longAddressValid  :1;  //1 = long address valid,
                                        //0 = long address unknown
            BYTE shortAddressValid :1;  //1 = short address valid,
                                        //0 = short address unknown
            BYTE FinishJoin        :1;  //1 = already finish joining procedure,
                                        //0 = in the process of join
            BYTE isFamily          :1;  //1 = family member (parent/child),
                                        //0 = not family
            BYTE filler            :1;
            BYTE isValid           :1;  //1 = this entry is valid,
                                        //0 = this entry is not valid
        } bits;
    #elif defined PROTOCOL_MIWI
    //Juan: In order to deal with the information of all MiWi interfaces, it has
    //been modified. Some bits have been rearranged and others have been added.
    //MIWITasks and other MiWi functions will have to manage this new flags.
        WORD Val;
        BYTE byte[2];
        struct _CONNECTION_STATUS_bits {
            //First byte...
            ...
            BYTE short0434AddrValid :1; //1 = short address valid,
                                        //0 = short address unknown
            BYTE is0434Family       :1; //1 = family member (parent/child),
                                        //0 = not family.
            BYTE direct0434Conn     :1; //1 = can talk to this device directly,
                                        //0 = must route to this device
            BYTE Finish0434Join     :1; //1 = already finish joining procedure,
                                        //0 = in the process of join
            //Second byte...
            BYTE short0868AddrValid :1; //1 = short address valid,
                                        //0 = short address unknown
            BYTE is0868Family       :1; //1 = family member (parent/child),
                                        //0 = not family.
            BYTE direct0868Conn     :1; //1 = can talk to this device directly,
                                        //0 = must route to this device
            BYTE Finish0868Join     :1; //1 = already finish joining procedure,
                                        //0 = in the process of join

            BYTE short2400AddrValid :1; //1 = short address valid,
                                        //0 = short address unknown
            BYTE is2400Family       :1; //1 = family member (parent/child),
                                        //0 = not family.
            BYTE direct2400Conn     :1; //1 = can talk to this device directly,
                                        //0 = must route to this device
            BYTE Finish2400Join     :1; //1 = already finish joining procedure,
                                        //0 = in the process of join
        } bits;
    #endif
    }CONNECTION_STATUS;
     ...
     *      This structure contains device information about the peer device of
     *      current node. It is the element structure for connection table. Due
     *      to the bank limitation in PIC18 MCU architecture, the size of
     *      CONNECTION_ENTRY must be dividable by 256 in case the array is
     *      across the bank. In this case, the user need to make sure that there
     *      is no problem
     **************************************************************************/
    typedef struct __CONNECTION_ENTRY
    {
        BYTE MiWiInterfaces;        //MiWi interfaces linked with this entry.
        #if !defined(PROTOCOL_P2P)
            //WORD_VAL PANID;               // PAN Identifier of the peer device.
                                            // May not necessary in P2P protocol
            // Alternative addr. of peer device. Not necessary in P2P protocol.
            //WORD_VAL AltAddress;
            // JUAN: One PANID and AltAddress per MiWi interface.
            #if defined MIWI_0434_RI
                WORD_VAL PAN0434ID;
                WORD_VAL MIWI0434AltAddress;
            #endif
            #if defined MIWI_0868_RI
                WORD_VAL PAN0868ID;
                WORD_VAL MIWI0868AltAddress;
            #endif
            #if defined MIWI_2400_RI
                WORD_VAL PAN2400ID;
                WORD_VAL MIWI2400AltAddress;
            #endif
        #endif
        ...
        CONNECTION_STATUS status;
            ...
            //Additional Node ID infomation, if defined in application layer
        #endif
    } CONNECTION_ENTRY;
    ...
    typedef struct {
        ...
        union{
            ...
            struct{
                ...
                BYTE Sleep      :1; // Whether the responding device goes to
                                    // sleep when idle
                BYTE SecurityEn :1; // Whether the responding device is capable
                                    // of securing the data
                BYTE RepeatEn   :1; // Whether the responding device allow repeat
                BYTE AllowJoin  :1; // Whether the responding device allows
                                    // other device to join
                BYTE Direct     :1; // Whether the responding device in radio
                                    // range or through a repeater
                BYTE altSrcAddr :1; // Whether the Address is alternative
                                    // network address or permanent address
            } bits;
        } Capability;
        #if ADDITIONAL_NODE_ID_SIZE > 0
            BYTE        PeerInfo[ADDITIONAL_NODE_ID_SIZE];
    ...
    typedef struct{
        union{
            ...
            struct{
                ...
                BYTE altSrcAddr: 1; // 1: source address is alternative network
                                    //    address
            } bits;
        } flags;
    ...
    //MICROCHIP - LSI Node: variables for each transceiver (prefix name added)
    #if defined MRF24J40
        extern RECEIVED_MESSAGE MIWI2400_rxMessage;
        extern BYTE MIWI2400_currentChannel;
    #endif
    #if defined MRF49XA_1
        #if defined MRF49XA_1_IN_434
            extern RECEIVED_MESSAGE MIWI0434_rxMessage;
            extern BYTE MIWI0434_currentChannel;
        #elif defined MRF49XA_1_IN_868
            extern RECEIVED_MESSAGE MIWI0868_rxMessage;
            extern BYTE MIWI0868_currentChannel;
        #endif
    #endif
...
//--------------------------- FUNCTION PROTOTYPES ----------------------------//
// MICROCHIP - LSI Node: Generally speaking, a new parameter has been added   //
// to the original functions, in order to determine which transceiver (or     //
// miwi_band, because every MiWi transceiver must be operating in a different //
// band) are they refering to.                                                //
//----------------------------------------------------------------------------//

    /***************************************************************************
     * Function:    void MiApp_ProtocolInit(BOOL bNetworkFreezer)
     * Summary:     This function initialize the Microchip proprietary wireless
     *              protocol
     * Description: This is the primary user interface function to initialize
     *              the Microchip proprietary wireless protocol, which is chosen
     *              by the application layer. Usually, this function must be
     *              called after the hardware initialization, before any other
     *              MiApp interface can be called.
     * PreCondition:Hardware initialization has been done.
     * Parameters:  BOOL bNetworkFreezer - Network Freezer is a feature for
     *                                     Microchip proprietary protocol. When
     *                                     this feature is enbaled, all critical
     *                                     network information can be saved in
     *                                     nonvotile memory, so that after power
     *                                     cycle, the network can be restored to
     *                                     its original state before the power
     *                                     lost without any communication. When
     *                                     Network Freezer feature is enabled
     *                                     and the input parameter of
     *                                     bNetworkFreezer is TRUE, the
     *                                     initialization process will restore
     *                                     the network paramaters.
     * Returns:     None
     * Example:
     *      <code>
     *      HardwareInit();
    ...
    /***************************************************************************
     * Function:    BOOL MiApp_SetChannel(BYTE Channel)
     * Summary:     This function set the operating channel for the RF
     *              transceiver
     * Description: This is the primary user interface function to set the
     *              operating channel for the RF transceiver. Valid channels are
     *              from 0 to 31. Depends on the RF transceiver, its frequency
     *              band, data rate and other settings, not all channels are
     *              available. If input channel is not available under the
     *              current condition, the current operating channel will not be
     *              changed, and the return value will be FALSE. Otherwise, the
     *              return value will be TRUE.
     * PreCondition:Protocol initialization has been done.
     * Parameters:  BYTE Channel - The channel to as the future operating
     *                             channel. Channels higher than 31 are invalid.
     *                             Valid channels between 0-31 depends on a lot
     *                             of factors
     * Returns:     a boolean to indicate if channel change has been performed
     *              successfully
     * Example:
     *      <code>
    ...
    /***************************************************************************
     * Function:    BOOL MiApp_StartConnection(BYTE Mode, BYTE ScanDuration,
     *                                         DWORD ChannelMap)
     * Summary:     This function start a PAN without connected to any other
     *              devices.
     * Description: This is the primary user interface function for the
     *              application layer to a PAN. Usually, this fucntion is called
     *              by the PAN Coordinator who is the first in the PAN. The PAN
     *              Coordinator may start the PAN after a noise scan if
     *              specified in the input mode.
     * PreCondition:Protocol initialization has been done.
     * Parameters:  BYTE Mode - Whether to start a PAN after a noise scan.
     *                          Possible modes are
     *                  * START_CONN_DIRECT     Start PAN directly without noise
     *                                          scan
     *                  * START_CONN_ENERGY_SCN Perform an energy scan first,
     *                                          then start the PAN on the
     *                                          channel with least noise.
     *                  * START_CONN_CS_SCN     Perform a carrier-sense scan
     *                                          first, then start the PAN on the
     *                                          channel with least noise.
     ...
     *                                  ScanDuration is discarded if the
     ...
     *                                 and channel 1. ChannelMap is discarded if
     *                                 the connection mode is START_CONN_DIRECT.
     * Returns:     a boolean to indicate if PAN has been started successfully.
     * Example:
     *      <code>
     *      // start the PAN on the least noisy channel after scanning all
     *      //possible channels.
     ...
     * Remarks:     None
     **************************************************************************/
    BYTE MiApp_SearchConnection(BYTE ScanDuration, DWORD ChannelMap, miwi_band mb);
     ...
     *                                     ActiveScanResults array, if a
     *                                     predefined device is targeted. If the
     *                                     value of ActiveScanIndex is 0xFF, the
     *                                     protocol stack will try to establish
     *                                     a connection with any device.
     ...
     *                  * CONN_MODE_DIRECT   Establish a connection within radio
     *                                       range.
     *                  * CONN_MODE_INDIRECT Establish a virtual connection with
     *                                       a device that may be in or out of
     *                                       the radio range.This mode sometimes
     *                                       is called cluster socket, which is
     *                                       only valid for network protocol.
     *                                       The PAN Coordinator will be
     *                                       involved to establish a virtual
     ...
     **************************************************************************/
    BYTE MiApp_EstablishConnection(BYTE ActiveScanIndex, BYTE Mode, miwi_band mb);
     ...
     * Remarks:     None
     **************************************************************************/
    void    MiApp_RemoveConnection(BYTE ConnectionIndex, miwi_band mb);
     ...
     * Description: This is the primary user interface function for the
     *              application layer to configure the way that the host device
     *              accept connection request.
     * PreCondition:Protocol initialization has been done.
     * Parameters:  BYTE Mode - The mode to accept connection request. The
     *                          privilege for those modes decreases gradually as
     *                          defined. The higher privilege mode has all the
     *                          rights of the lower privilege modes. The
     *                          possible modes are
     *                  * ENABLE_ALL_CONN        Enable response to all
     *                                           connection request
     *                  * ENABLE_PREV_CONN       Enable response to connection
     *                                           request from device already in
     *                                           the connection table.
     *                  * ENABLE_ACTIVE_SCAN_RSP Enable response to active scan
     *                                           only
     *                  * DISABLE_ALL_CONN       Disable response to connection
     *                                           request, including an acitve
     *                                           scan request.
     * Returns:     None
     * Example:
     *      <code>
     ...
     * Remarks:     None
     **************************************************************************/
    void MiApp_ConnectionMode(BYTE Mode, miwi_band mb);

    /***************************************************************************
     * Function:    MiApp_FlushTx()
     * Description: This macro reset the pointer of the TX buffer. This function
     *              is usually called before filling application payload.
     * PreCondition: Protocol initialization has been done.
     * Parameters:  None
     * Returns:     None
     * Example:
    ...
    /***************************************************************************
     * Function:    MiApp_WriteData()
     * Description: This macro writes one byte of application payload to the TX
     *              buffer.
     * PreCondition:Protocol initialization has been done. MiApp_FlushTx() has
     *              been called before calling the first MiApp_WriteData for the
     *              first time.
     * Parameters:  BYTE a - One byte of application payload to be written to
     *                       the TX buffer
     * Returns:     None
     * Example:
     *      <code>
     ...
     * Remarks:     None
     **************************************************************************/
    void MiApp_WriteData(BYTE data, miwi_band mb);     //Added mb parameter
    // MICROCHIP - LSI Node: This function was originally defined as a macro.
    // Original code:    #define MiApp_WriteData(a)   TxBuffer[TxData++] = a

    /***************************************************************************
     * Function:    BOOL MiApp_BroadcastPacket(BOOL SecEn )
     * Summary:     This function broadcast a message in the TxBuffer.
     * Description: This is the primary user interface function for the
     *              application layer to broadcast a message. The application
     *              payload is filled in the global char array TxBuffer.
     * PreCondition:Protocol initialization has been done.
     * Parameters:  BOOL SecEn - The boolean indicates if the application
     *                           payload needs to be secured before transmission.
     ...
     * Remarks:     None
     **************************************************************************/
    BOOL MiApp_BroadcastPacket(BOOL SecEn, miwi_band mb);

    /***************************************************************************
     * Function:    BOOL MiApp_UnicastConnection(BYTE ConnectionIndex, BOOL SecEn)
     * Summary:     This function unicast a message in the TxBuffer to the
     *              device with the input ConnectionIndex in the connection
     *              table.
     * Description: This is one of the primary user interface functions for the
     *              application layer to unicast a message. The destination
     *              device is in the connection table specified by the input
     *              parameter ConnectionIndex. The application payload is filled
     *              in the global char array TxBuffer.
     * PreCondition:Protocol initialization has been done. The input parameter
     *              ConnectionIndex points to a valid peer device in the
     *              connection table.
     * Parameters:  BYTE ConnectionIndex - The index of the destination device
     *                                     in the connection table.
     *              BOOL SecEn - The boolean indicates if the application
     *                           payload needs to be secured before transmission
     * Returns:     A boolean to indicates if the unicast procedure is
     *              succcessful.
     * Example:
     *      <code>
     *      // Secure and then unicast the message stored in TxBuffer to the
     *      // first device in the connection table
     *      MiApp_UnicastConnection(0, TRUE);
     *      </code>
     * Remarks:     None
     **************************************************************************/
    BOOL MiApp_UnicastConnection(BYTE ConnectionIndex, BOOL SecEn, miwi_band mb);

    /***************************************************************************
     * Function:    BOOL MiApp_UnicastAddress(BYTE *DestinationAddress,
     *                                        BOOL PermanentAddr, BOOL SecEn)
     * Summary:     This function unicast a message in the TxBuffer to the
     *              device with DestinationAddress
     * Description: This is one of the primary user interface functions for the
     *              application layer to unicast a message. The destination
     *              device is specified by the input parameter
     *              DestinationAddress. The application payload is filled in the
     *              global char array TxBuffer.
     * PreCondition:Protocol initialization has been done.
     * Parameters:  BYTE *DestinationAddress - The destination address of the
     *                                         unicast
     *              BOOL PermanentAddr - The boolean to indicate if the
     *                                   destination address above is a
     *                                   permanent address or alternative
     *                                   network address. This parameter is only
     ...
     * Remarks:     None
     **************************************************************************/
    BOOL MiApp_UnicastAddress (BYTE *DestinationAddress, BOOL PermanentAddr,
                               BOOL SecEn, miwi_band mb);
    ...
    /***************************************************************************
     * Function:    BOOL    MiApp_MessageAvailable(void)
     * Summary:     This function return a boolean if a message is available for
     *              the application
     * Description: This is the primary user interface functions for the
     *              application layer to call the Microchip proprietary protocol
     *              stack to check if a message is available for the application
     *              The function will call the protocol stack state machine to
     *              keep the stack running. It is expected that this function
     *              should be called periodically in the application. In case a
     *              message is available, all information related to the
     *              recevied message will be stored in the global variable
     *              RxMessage in the format of RECEIVED_MESSAGE.
     * PreCondition:Protocol initialization has been done.
     * Parameters:  None
     * Returns:     A boolean to indicates if a message is available for
     *              application.
     * Example:
     *      <code>
    ...
    /***************************************************************************
     * Function:    void MiApp_DiscardMessage(void)
     * Summary:     This function discard the current message for the
     *              application and notify the protocol layer that it is ready
     *              to receive the next message.
     * Description: This is the primary user interface functions for the
     *              application layer to discard the current active message,
     *              release the system resources used and ready to receive the
     *              next message. It is must be called after finish handling the
     *              message, otherwise, no further message can be received.
     * PreCondition:Protocol initialization has been done. A message has been
    ...
    BYTE MiApp_NoiseDetection (DWORD ChannelMap, BYTE ScanDuration,
                               BYTE DetectionMode, OUTPUT BYTE *NoiseLevel,
                               miwi_band mb);
     ...
     * Summary:     This function tries to start a channel hopping (frequency
     *              agility) procedure
     * Description: This is the primary user interface function for the
     *              application to do energy scan to locate the channel with
     *              least noise.If the channel is not current operating channel,
     *              process of channel hopping will be started.
     ...
     *              with least noise
     **************************************************************************/
    BOOL MiApp_InitChannelHopping(DWORD ChannelMap, miwi_band mb);
     ...
     * PreCondition:Transceiver has been initialized
     * Parameters:  DWORD ChannelMap - The bit map of channels to perform noise
     *              scan. The 32-bit double word parameter use one bit to
     *              represent corresponding channels from 0 to 31. For instance,
     *              0x00000003 represent to scan channel 0 and channel 1.
